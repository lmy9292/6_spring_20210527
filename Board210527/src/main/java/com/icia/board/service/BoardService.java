...
public class BoardService {
	...
	public ModelAndView boardPaging(int page) {
		...
		//Math.ceil: 올림처리를 해주는 역활 3.9=>4, 4.1=>5
		 ...
		 * 하단의 페이지 목록은 1,2,3,4 만 노출되면 되기 때문에 아래와 같은 식을 통해 endPage 값을 조정. 
		 */
		if(endPage > maxPage)
			endPage = maxPage;
	...
	public ModelAndView boardSearch(String searchType, String keyword) {
		...
		//Map은 두가지 타입을 한가지로 묶을 때 사용
		Map<String, String> searchMap = new HashMap<String, String>();
		//searchMap.put:스트링 타입으로 와야한다
		...
		//<c:forEach var="board" items="${boardList}"> boardlist.jsp에 있는 것과 addObject과 똑같이 쓴다
